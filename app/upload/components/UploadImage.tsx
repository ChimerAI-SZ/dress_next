...
function Page({ onParamsUpdate }: TypesClothingProps) {
  ...
  return (
    <Box alignItems="center" justifyContent="center" mt="0.8rem" width="full" position="relative" bg={"#FFFFFF"} borderRadius={"0.5rem"} px="0.75rem" pb={"0.75rem"} mb={"0.5rem"}>
      ...
      <Flex width="100%" height="19.38rem" background="#F5F5F5" borderRadius="0.5rem" alignItems="center" justifyContent="center" flexFlow="column">
        {isUploading ? (
          ...
          <Flex h="100%" w="100%" justifyContent="center" alignItems="center" position="relative" overflow="hidden">
            <Image src={uploadedUrl} alt="Background image" h="100%" w="100%" objectFit="cover" filter="blur(10px)" position="absolute" top={0} left={0} zIndex={0} />
            <Image src={uploadedUrl} alt="Foreground image" h="100%" objectFit="contain" zIndex={1} />
            <Box as="label">
              <Image src={ReUpload.src} alt="Re-upload icon" h="1.1rem" w="1.13rem" objectFit="cover" position={"absolute"} zIndex={2} bottom="0.75rem" right="0.75rem" />
              <input type="file" multiple accept="image/*" style={{ display: "none" }} onChange={handleFileUpload}></input>
            </Box>
          </Flex>
        ) : (
          <>
            <Button w="8.19rem" h="2rem" borderRadius="1rem" bg="rgba(255,255,255,0.5)" border="0.06rem solid #EE3939" as="label" cursor="pointer" gap={"0.2rem"}>
            ...
            <Text fontFamily="PingFangSC, PingFang SC" fontWeight="400" fontSize="0.81rem" color="#BFBFBF" mt="0.38rem">
